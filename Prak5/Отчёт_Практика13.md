...
Реализованная архитектура соответствует принципам Clean Architecture:
...
✅ **Retrofit 2.9.0** - успешно применён для выполнения GET-запросов к TMDB API, десериализации JSON-ответов в Java-объекты с использованием Gson.

✅ **Glide 4.16.0** - интегрирован для асинхронной загрузки и кэширования изображений из сети с поддержкой трансформаций (centerCrop, circleCrop).

✅ **OkHttp Logging Interceptor** - настроен для детального логирования HTTP-запросов и ответов, что упрощает отладку.

✅ **Clean Architecture** - сохранена при интеграции библиотек, обеспечена изоляция слоёв через DTO и Mapper.

✅ **Обработка ошибок** - реализован fallback механизм с использованием FakeMovieApi при отсутствии сети.

✅ **MovieProject** успешно загружает данные о фильмах из TMDB API и отображает постеры.

✅ **Askon** успешно загружает и отображает круглые аватары экспертов с помощью Glide.
...
- Gson (JSON converter)
- MVVM + LiveData (архитектурный паттерн)
- Clean Architecture (разделение на модули)
...
└── src/main/
    ...
    ├── java/ru/mirea/yudaev/retrofitapp/
    │   ├── RetrofitDemoActivity.java    # Activity с RecyclerView
    │   ├── MovieDto.java                # DTO для фильма
    │   ├── MoviesResponse.java          # Обёртка для списка
    │   ├── MovieApiService.java         # Retrofit интерфейс
    │   ├── MovieApiClient.java          # Клиент с логированием
    │   └── MovieAdapter.java            # Адаптер для RecyclerView
    └── res/
        ├── layout/
        │   ├── activity_retrofit_demo.xml   # Главная разметка
        │   └── item_movie.xml               # Карточка фильма
        └── values/
            └── strings.xml
```
...
✅ **Автономность** - модуль может запускаться отдельно от главного приложения

✅ **Упрощённая архитектура** - без разделения на domain/data, всё в одном модуле

✅ **Логирование HTTP** - OkHttp Logging Interceptor для отладки запросов

✅ **Glide интеграция** - загрузка постеров с placeholder и обработкой ошибок

✅ **Material Design** - использование CardView и ConstraintLayout для современного UI

### Запуск модуля

Модуль подключен в `settings.gradle.kts`:
```kotlin
include(":retrofitapp")
```

Для запуска: `./gradlew :retrofitapp:assembleDebug`
